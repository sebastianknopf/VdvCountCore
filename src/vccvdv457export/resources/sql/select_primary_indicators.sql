WITH counted_stops AS
    ...
    (SELECT operation_day,
     ...
     FROM grouped_matched_passenger_counting_events
     GROUP BY operation_day,
              trip_id,
              vehicle_id)
SELECT g.operation_day,
       g.trip_id,
       g.vehicle_id,
       g.device_id
FROM grouped_matched_passenger_counting_events g
JOIN primary_indicators p ON g.operation_day = p.operation_day
AND g.trip_id = p.trip_id
AND g.vehicle_id = p.vehicle_id
AND g.num_matched_pce = p.max_pce
ORDER BY g.operation_day,
         g.trip_id,
         g.vehicle_id;